...
FOCUSED_CRATES := use-version use-crate
FACADE_CRATE := use-rust
...
help:
	@printf "%s\n" \
		...
		"deny                           Run cargo-deny" \
		"sbom                           Generate a CycloneDX SBOM for use-rust" \
		"publish-dry-run-focused        List package contents and dry-run publish focused crates" \
		"publish-dry-run-facade         Dry-run publish use-rust after crates.io propagation" \
...
publish-dry-run-facade:
	cargo package --list -p $(FACADE_CRATE)
